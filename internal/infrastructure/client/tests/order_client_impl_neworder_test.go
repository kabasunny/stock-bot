...
func TestOrderClientImpl_NewOrderWithPost_Cases(t *testing.T) {
	// テスト用の TachibanaClient を作成
	c := client.CreateTestClient(t)
	...
	_, err := c.LoginWithPost(context.Background(), loginReq) // Use LoginWithPost
	assert.NoError(t, err)
	...
	t.Run("正常系 (POST): 現物 買い注文 (成行、特定口座) が成功すること", func(t *testing.T) {
		...
		res, err := c.NewOrderWithPost(context.Background(), orderReq) // Use NewOrderWithPost
		assert.NoError(t, err)
		assert.NotNil(t, res)
		if res != nil {
			assert.Equal(t, "0", res.ResultCode)
